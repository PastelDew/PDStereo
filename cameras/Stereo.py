...
class Stereo():
    ...
    def stereoCalibrate(imgSize, rectifyScale = 0): # 0=full crop, 1=no crop
        if not (os.path.exists(cam_params_left_path)\
    ...
    def stereoMatching(leftFrame, rightFrame, filename='untitled', savePointClound=False, rectifyScale=0):
        imgSize = leftFrame.shape[:2][::-1]
        if not self.isStereoCalibrated:
            assert self.stereoCalibrate(imgSize, rectifyScale)
